...
public final class HomePresenterImpl extends BasePresenter implements HomePresenter {
    ...
    private void onGetHistorySuccess(final List<HistoryApiResponse> historyApiResponses) {
        if (view != null){
            final List<HistoryPlan> planList = new ArrayList<>(historyApiResponses.size());
            int i = 0;
            for (HistoryApiResponse response : historyApiResponses){
                planList.add(new HistoryPlan(String.valueOf(++i), response.store, response.address, response.checkoutDate, response.total));
            }
